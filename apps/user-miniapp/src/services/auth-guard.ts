...
export function hasUserLogin() {
  return Boolean(getUserToken());
}

export function requireUserLogin(url = currentPageUrl()) {
  if (hasUserLogin() || isPublicPage(url)) {
    return true;
  }
...
export function resolveUserLoginRedirect(rawRedirect?: string) {
  if (!rawRedirect) {
    return "/pages/home/index";
  }
...
export function goAfterUserLogin(rawRedirect?: string) {
  const target = resolveUserLoginRedirect(rawRedirect);
